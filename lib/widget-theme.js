...
export function themeToCssVars(theme, { dark = false } = {}) {
  ...
  return {
     ...
     * Placement.
     *
     * The configured offsets are a preference, not a promise: `MAX_OFFSET` is
     * 200px, which on a small screen would push the launcher — and with it the
     * panel — clean off the viewport. Each offset is therefore capped in CSS
     * rather than JS, against the viewport it is actually rendering into, so
     * the cap re-evaluates on rotate and resize instead of at render time.
     *
     * The reserves (18rem across, 26rem down) are what the panel needs to stay
     * usable; a desktop is nowhere near them, so a normal setting passes
     * through untouched.
     */
    "--wp-offset-x": `min(${t.offsetX}px, max(0.75rem, (100vw - 18rem) / 2))`,
    "--wp-offset-y": `min(${t.offsetY}px, max(0.75rem, 100dvh - 26rem))`,

    // One side is `auto` so the other wins. The safe-area inset is added to
    // whichever side is live, so the launcher clears the rounded corners and
    // the home indicator without the offset itself having to know about them.
    "--wp-left":
      t.side === "left"
        ? "calc(var(--wp-offset-x) + var(--wp-safe-l))"
        : "auto",
    "--wp-right":
      t.side === "left"
        ? "auto"
        : "calc(var(--wp-offset-x) + var(--wp-safe-r))",

    /*
     * Where the panel's bottom edge sits: clear of the launcher, plus a gap.
     *
     * Expressed in CSS rather than as `offsetY + 72`, because that 72 was the
     * launcher's 56px plus a 16px gap frozen into JavaScript — it could not
     * follow a launcher that changes size at a breakpoint, and would have
     * silently left the panel overlapping it.
     */
    "--wp-panel-offset-y":
      "calc(var(--wp-offset-y) + var(--wp-safe-b) + var(--wp-launcher-size) + 1rem)",
